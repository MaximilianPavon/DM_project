...
print('load graph:\t', round((time.time() - start),1), ' s')

LSCC = analysis_nx.calculate_largest_strongly_connected_comp(g)
print('calculate LSCC:\t', round((time.time() - start),1), ' s')

LWCC = analysis_nx.calculate_largest_weakly_connected_comp(g)
print('calculate LWCC:\t', round((time.time() - start),1), ' s')
...
print('caluclate distances LSCC:\t', round((time.time() - start),1), ' s')

LWCC_dist = analysis_nx.compute_shortest_path_distances_parallel_mp(LWCC)
print('caluclate distances LWCC:\t',round((time.time() - start),1), ' s')
...
print('complete code:\t', round((time.time() - start),1), ' s')
